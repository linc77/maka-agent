# 01 — Alma's REST API + self-referential `alma-operator` agent

> Source-grounded against `~/Downloads/alma-re/readable/main.js`.
> Round 4 opens with a surface neither round 1, 2, nor 3 touched:
> alma exposes a localhost REST API that the LLM agent can call
> via `curl` to **operate the app itself** — change theme, swap
> models, add providers, etc. The `alma-operator` subagent is the
     ...
     setImmediate(() => {
       try {
         const e = (function (port) {
           const t = U.join(n.getPath("home"), ".config", "alma", "api-spec.md");
           const o = `# Alma API Specification\n…`; // big template literal
...
function Wh(e) {
  const t = (function () {
    const e = F.join(I.homedir(), ".config", "alma", "api-spec.md");
...
This is **the same file the alma-operator agent reads**. The agent
...
## `alma-operator` agent shape

`main.js:20447`:

```js
"alma-operator": ["Bash", "Read"]
```
...
> Your goal is to read and modify Alma's runtime configuration
> via its REST API.
>
> ## API Specification
> The complete API specification is available at:
> `~/.config/alma/api-spec.md`
>
...
>
> ## Workflow
> 1. **First:** Read ~/.config/alma/api-spec.md to understand the
>    complete API
...
Alma's Task tool description (`main.js:22269+`) describes
`alma-operator` as:

> Use this agent to read or modify Alma's runtime configuration.
...
> theme" or "Update your settings", they are referring to Alma
> (the app). The agent reads `~/.config/alma/api-spec.md` for the
> actual API port.

The framing "When users say 'you', they mean Alma" is a tell:
...
- No `alma-operator`-style agent that reads the spec and
...
- Is there a CSRF risk? The server binds to 127.0.0.1 only, but
  ...
  curl it without auth. Does alma have a session token? Looking
...
- Round 1: [overall agent loop section](../alma-deep-dive-yuejing-2026-05-31/)
...
- Round 2: [`07-subagent-orchestration.md`](../alma-deep-dive-yuejing-round-2/07-subagent-orchestration.md)
  — alma-operator is one of the 7 specialists; only it has the
...
- Round 3: [`01-skills-system.md`](../alma-deep-dive-yuejing-round-3/01-skills-system.md)
  ...
  to dark" → load skill → invoke alma-operator).
